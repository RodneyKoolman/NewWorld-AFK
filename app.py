...
def wait(range_start, range_stop):
    ...
    while wait_in_seconds > 0:
        ...
        time.sleep(0.1)
        tooltip_label["text"] = "Current action: waiting {} seconds".format(
            wait_in_seconds
        )
...
info_label = Label(
    frame_top,
    text="Press start and click inside the game once",
...
tooltip_label = Label(
    frame_middle,
    text="Tip: use 'tab' in-game to get your mouse pointer",
...
exit_button = Button(
    frame_bottom, text="Exit", fg="red", font=("Helvetica", "9", "bold"), command=exit
)
